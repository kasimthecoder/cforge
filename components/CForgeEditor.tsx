...
type Project = { id: string; title: string; code: string };
...
export default function CForgeEditor({ project, isAuthenticated }: { project?: Project; isAuthenticated: boolean }) {
  ...
  }, [code, isAuthenticated, projectId, title]);
  ...
  const run = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/run', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sourceCode: code, stdin }) });
      ...
      <main className="editor-layout">
        <section className="code-panel">
          <div className="code-toolbar">
            ...
            <label className="project-title-label" htmlFor="project-title">Project name</label>
            <input
              id="project-title"
          ...
          <div className="editor-status"><span>C / GNU 9.2.0</span><span>Tab inserts spaces</span><span>UTF-8</span></div>
        </section>
